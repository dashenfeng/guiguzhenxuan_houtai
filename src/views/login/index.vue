...
let loading = ref(false) //定义变量控制按钮加载效果
let loginForm = reactive({ username: 'admin', password: '111111' }) //收集账号与密码的数据
//登录按钮回调
const login = async () => {
  await loginForms.value.validate() //保证全部表单相校验通过再发请求
  loading.value = true //加载效果:开始加载
  try {
    //保证登录成功
    ...
    loading.value = false//登录成功加载效果也消失
  } catch (error) {
    loading.value = false//登录失败加载效果消息
    //登录失败的提示信息
    ElNotification({
      type: 'error',
